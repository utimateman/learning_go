...
func parseSignaturesFile(path string) (map[string]string, error) {
	...
	scanner := bufio.NewScanner(file) // go over the files
	for lnum :=1; scanner.Scan(); lnum++ { // while scan true
		// ae;ksdfaklsjdf nasa-05.log
		fields := strings.Fields(scanner.Text()) //split
		if len(fields) != 2 {
